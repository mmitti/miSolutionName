...
namespace WindowColor
{
    ...
    public sealed class WindowColorPackage : AsyncPackage
    {
        ...
        protected override async Task InitializeAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
        {
            ...
            DTE.Events.WindowEvents.WindowCreated += WindowEvent;
            DTE.Events.WindowEvents.WindowActivated += WindowEventActivated;
            DTE.Events.WindowEvents.WindowClosing += WindowEvent;
            
                
        }
        DTE2 DTE;

        private void WindowEvent(EnvDTE.Window Window)
        {
            int ii = 0;
        }
        private void WindowEventActivated(EnvDTE.Window Window, EnvDTE.Window Window2)
        {
            int ii = 0;
        }
        ...
        T GetElement<T>(string name = null) where T : class
        {
            DependencyObject find(DependencyObject r)
            {
            ...
            return find(System.Windows.Application.Current.MainWindow) as T;
        }
        private void HandleOpenSolution(object sender = null, EventArgs e = null)
        {
            Application.Current.Dispatcher.Invoke(() => {
                var c = GetElement<Path>("VectorIcon");
